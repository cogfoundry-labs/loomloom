...
- For a Profile Step, read its fixed input/output interface and live eligible-model set from `template-spec authoring-context`; normal templates write the stable `profileId` and omit `profileRevision`. Dynamic Profiles expose their interface under `definition`; legacy static Profiles expose it through top-level `inputPorts` and `output`. Do not fabricate a `subjectRevisionId`.
...
- For `dynamic=true`, use `operations.defaultModelId` only when `defaultModelAvailable=true` and the model is in `eligibleModels`. For a legacy static Profile where `dynamic` is absent or false, choose an explicit default from `eligibleModels`; missing `operations` and `defaultModelAvailable` are expected and do not make the Profile unavailable.
