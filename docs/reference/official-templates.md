...
Official templates are platform-maintained templates (for example, `text-v1`); their availability depends on the loomloom execution platform you are connected to. Always use `loomloom template list` to view the latest available templates.

For exact field keys, labels, enum values, versions, and schema details, run:
...
| `cancelled` | Run was canceled. |

You can monitor and manage your loomloom runs in the following ways:

- **Monitor runs from the CLI:** Use `loomloom run get <run-id>` or `loomloom run watch <run-id>` to check execution status.
- **Manage your account:** Use the console of your loomloom execution platform to check account balance, recharge credits, and view transaction status.