# loomloom CLI command reference
...
For how to install, configure, and uninstall loomloom CLI on your local development machine using different options, check the [installation guide](../quick-start/installation.md).
...
| `loomloom template-spec authoring-context` | Show current Capability Profiles, including fixed input/output interfaces, default-model availability, and the live eligible-model set. |
...
Install Skill Package ZIPs as local tools for AI agents. The older local-wrapper commands remain available as a separate compatibility workflow.

### Install Skill Package ZIPs

Use these commands for Market and official-template installation. They download the ZIP when needed, verify it, and extract it beneath the current Agent's Skill root; an up-to-date installation is left unchanged.
...
These commands generate local calling instructions, not download ZIPs. The private-template command can create a wrapper for an exact template version without an existing Skill Package ZIP. ZIP download commands do not replace that capability.

| Command | Description |
|---|---|
| `loomloom skill install market <listing-id> --agent <agent> --output-dir <skill-dir>` | Compatibility: generate a local wrapper for a Market Listing. |
| `loomloom skill install template-spec <template-id> <version-id> --agent <agent> --output-dir <skill-dir>` | Compatibility: generate a local wrapper pinned to a private template version. |
| `loomloom skill uninstall --dir <skill-dir>` | Compatibility: remove a wrapper generated by the two commands above; does not support installed ZIP packages. |
...
- For wrapper uninstall, run `loomloom skill uninstall --dir <skill-dir> --dry-run --output json` first. The command requires valid `loomloom-skill.json` metadata. `--force` permits extra files in a valid wrapper directory; it does not add support for ZIP packages installed by `skill package install`.
