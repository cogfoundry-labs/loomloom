...
These `skill package install` commands install the backend-provided package. The older wrapper commands remain available as a separate compatibility workflow: `skill install market` generates local calling instructions for a Listing, while `skill install template-spec` does so for an exact private template version without requiring an existing ZIP. These instructions call hosted execution; they do not download its workflow for local execution. `skill uninstall` only removes wrappers generated by those commands, not packages installed by `skill package install`. Downloading a private template's existing ZIP does not replace private-wrapper generation.
