...
git -C "$test_root/work" push origin release/v1.1.0 v1.1.0-beta.1 >/dev/null
git init --bare "$test_root/prerelease.git" >/dev/null
run_sync gitee "$test_root/prerelease.git" v1.1.0-beta.1 release/v1.1.0 >/dev/null
assert_ref "$test_root/prerelease.git" refs/heads/release/v1.1.0 "$release_branch_sha"
...
git -C "$test_root/work" push origin release/v1.1.0 v1.1.0-rc.1 >/dev/null
git init --bare "$test_root/annotated.git" >/dev/null
run_sync gitlab "$test_root/annotated.git" v1.1.0-rc.1 release/v1.1.0 >/dev/null
assert_ref "$test_root/annotated.git" refs/heads/release/v1.1.0 "$annotated_commit"
...
git -C "$test_root/annotated-conflict-work" push \
  ...
  "$release_branch_sha:refs/heads/release/v1.1.0" \
  refs/tags/v1.1.0-rc.1:refs/tags/v1.1.0-rc.1 >/dev/null
if run_sync \
  gitlab \
  "$test_root/annotated-conflict.git" \
  v1.1.0-rc.1 \
  release/v1.1.0 >/dev/null 2>&1; then
...
git -C "$test_root/work" push \
  ...
  "$annotated_commit:refs/heads/release/v1.1.0" \
  "$annotated_commit:refs/tags/v1.1.0-rc.1" >/dev/null
run_sync \
  gitlab \
  "$test_root/normalized-lightweight.git" \
  v1.1.0-rc.1 \
  release/v1.1.0 >/dev/null
assert_ref \
  "$test_root/normalized-lightweight.git" \
  refs/tags/v1.1.0-rc.1 \
  "$annotated_commit"
