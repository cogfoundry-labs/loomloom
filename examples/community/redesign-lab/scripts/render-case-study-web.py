...
.wrap{max-width:1920px;margin:0 auto;padding:0 24px;}
/* 1920px (not the old 900px) so the whole page -- not just the compare
   widget -- scales up together on a large screen: a human specifically
   wanted the Before/After compare section to reach a true 1920x1080 (its
   aspect-ratio:16/9 box) at full width, for clean full-HD screen
   recordings of the drag interaction, and asked for the rest of the page
   to match rather than have one wide section sit inside an otherwise
   narrow-column page. Actual flowing prose (.chapter-narrative,
   .loomloom-note below) still gets its own, much narrower max-width --
   a 1920px-wide paragraph is far past any reasonable line length and
   would just trade one "looks wrong" for another, not something this
   change was actually asking for. Non-text elements (headings, the
   compare widget, stat/fact grids, swatches) use the full wide column. */
a{color:var(--accent);}
