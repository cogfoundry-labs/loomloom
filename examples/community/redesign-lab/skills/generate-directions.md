...
**Build and show slices one at a time, progressively — never build all of
them silently and reveal them together.** This is rev 7's actual fix for two
things that used to go wrong at once: a combined comparison page (many
slices embedded as iframes in one file) routinely exceeded this environment's
real base64-payload rendering limit, and the "jump to another slice" links
built to work around a failed combined page turned out to be completely
inert regardless of how they were built (a `data:`-rendered local file is
blocked from navigating to another local file via an in-page click — see
`../references/approval-policy.md`'s "How gates are actually shown" for the
full finding). Real, separate browser tabs sidestep both problems at once:
no combined payload to exceed a limit, and no in-page link required, because
the human just switches tabs in the browser's own tab bar.
...
1. Build the real HTML file (per "Building each slice" above), with a real,
   ...
   colorway") — this is the only text added anywhere near the slice itself;
   the rendered page is the real deliverable, unwrapped, not a slice
   embedded inside some other meta-page. The browser's own tab bar shows
   this title, which is what actually lets the human tell tabs apart once
   several are open.
2. Run `mechanical-check.py` against it.
3. Open it in its own real browser tab, and tell the human it's ready in
   the *chat* message — "`current-fixed` is up in its own tab: [1-3 real
   notable facts, including any defects fixed] — mechanical-check: 11/11."
   One short message per slice, not saved up for the end. The facts and
   score live in this message, not baked into the page itself — with one
   option per tab there's no longer a multi-option page to scroll through
   and lose track of which is which, so there's nothing a printed header
   would add that the chat message doesn't already say.
4. Immediately start the next slice. The human can look at what's already
   open while this one builds — that's the point of doing this one at a
   time instead of batching everything before showing anything.

**The decision itself, once every slice in this pass is open, is an
`AskUserQuestion` call** — not a numbered list in plain chat. Options: each
slice by name (with its 1-3 notable facts as the option's description), plus
explicit flexibility options in the same call: "ask for more directions"
(agent picks 1-2 more, same selection method, opened the same progressive
way) and "let me pick one more myself" (name a `direction_variants` skill
not yet shown). Always include a "stop here" option — nothing already built
is lost. See `approval-policy.md`'s "How gates are actually shown" for why
this is the mechanism now (and what it replaced).
